...
def _template_with_flavor(
    features: dict, settings: dict | None = None
) -> SimpleNamespace:
...
async def test_settings_ride_the_block_uninterpreted():
    """How a feature behaves, for the cases a bool cannot express.

    Carried exactly as written — the engine has no idea what a nudge line
    is, and a key it has never heard of must reach the chunk unchanged.
    """
    block = await _surface_wire(
        _surface(),
        _template_with_flavor(
            {"show_viewed_product": True},
            {"nudge_lines": ["Try it on before you buy"], "unknown_key": 7},
        ),
        catalog_active="v2",
        ui_flavors=["commerce"],
    )
    assert block.flavor["ucp"].settings == {
        "nudge_lines": ["Try it on before you buy"],
        "unknown_key": 7,
    }


async def test_settings_default_to_empty_so_the_chunk_uses_its_own():
    """A template that says nothing about copy gets the compiled-in copy."""
    block = await _surface_wire(
        _surface(),
        _template_with_flavor({"show_viewed_product": True}),
        catalog_active="v2",
        ui_flavors=["commerce"],
    )
    assert block.flavor["ucp"].settings == {}
