...
def _template_flavor(template: object) -> Dict[str, FlavorWire]:
    ...
    for protocol, block in blocks.items():
        ...
        # verbatim.
        raw_settings = getattr(block, "settings", None)
        settings = dict(raw_settings) if isinstance(raw_settings, dict) else {}
        out[str(protocol)] = FlavorWire(
            connectors=connectors, features=features, settings=settings
        )
    return out
...
async def _read_upload(upload: UploadFile, max_bytes: int, label: str) -> bytes:
    ...
    Enforces the size after the framework has parsed the multipart body,
    so this is a per-route cap, not a guard against large bodies: that
    belongs at the proxy. ``label`` is capitalised for the 413 and
...
async def try_on_widget_handler(
    ...
    ownership → 410 ENDED → garment signature → reload cache →
    per-session cap → wallet → request_id claim.
    ...
    if claim == "running":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="This try-on is already in progress.",
        )
    if claim == "done":
        # Generated and charged, but the cached image has expired. The
        # widget drops this id on 404 and starts a new try-on.
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="This try-on has expired. Please try again.",
        )
    ...
    await finish_try_on_request(session_id, request_id)
    ...
    # Session state carries a counter only — never an image, never the
    # photo. The merge is shallow at the top level, so the whole try_on
    # block is rewritten each time.
    try:
        await upsert_agent_session_state_merge(
            session_id, {"try_on": {"count": used + 1}}
        )
    except Exception:
